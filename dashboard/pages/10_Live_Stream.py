...
<div style="background:linear-gradient(135deg,#0d1b2e,#1a2744);border-radius:16px;
    ...
    <h1 style="margin:0;color:white;font-size:28px;">Live KYC Stream</h1>
    <p style="color:#64748b;margin:6px 0 0 0;font-size:13px;">
        Simulated real-time KYC verification feed — deepfake detection + face match
    </p>
</div>
""", unsafe_allow_html=True)
...
if 'stream_stats' not in st.session_state:
    st.session_state.stream_stats = {'total': 0, 'approved': 0, 'rejected': 0, 'review': 0, 'deepfakes': 0}
...
with m4: st.metric("Review", st.session_state.stream_stats['review'])
...
def render_stream():
    ...
    with stream_placeholder.container():
        st.markdown("### Verification Feed")
        if not history:
            st.info("Click Start Stream to begin.")
            return
        ...
        for item in history:
            ...
            <div style="background:#0f172a;border-left:4px solid {color};
                ...
                <span style="color:{face_color};font-size:12px;">Face: {item['face_result']}</span>
                <span style="color:#a78bfa;font-size:12px;">Match: {item['match_result']}</span>
...
if st.session_state.stream_running:
    status_placeholder.markdown("<p style='color:#059669;font-size:13px;'>● Stream running</p>", unsafe_allow_html=True)
    ...
    status_placeholder.markdown("<p style='color:#64748b;font-size:13px;'>Stream paused</p>", unsafe_allow_html=True)