...
<div style="background:linear-gradient(135deg,#0d1b2e,#1a2744);border-radius:16px;
    ...
    <h1 style="margin:0;color:white;">🔬 GradCAM Forgery Analysis</h1>
    <p style="color:#64748b;margin:4px 0 0 0;">
    Visual explanation of WHERE the AI detected forgery — red regions indicate suspicious areas
    </p>
</div>
""", unsafe_allow_html=True)
...
<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:8px;padding:12px 16px;margin-bottom:16px;">
    ...
    WHICH pixels the AI model focused on when making its forgery decision. Red = high suspicion, 
...
tab1, tab2 = st.tabs(["🧬 Face Deepfake Analysis", "📄 Document Forgery Analysis"])

with tab1:
    st.markdown("### 🧬 Face Deepfake GradCAM")
    ...
    if face_file:
        ...
        if st.button("🔬 Analyze with GradCAM", type="primary", use_container_width=True, key="btn_face"):
            with st.spinner("Generating GradCAM visualization..."):
                try:
                    ...
                    color = '#dc2626' if is_deepfake else '#059669'
                    result_text = '🤖 DEEPFAKE DETECTED' if is_deepfake else '✅ AUTHENTIC FACE'
                    ...
                    with c3:
                        st.markdown("**Overlay (Original + Heatmap)**")
                        st.image(overlay, use_column_width=True)
                        st.caption("Shows exactly WHERE AI detected deepfake artifacts")

                    st.divider()
                    st.markdown("#### 📊 Confidence Breakdown")
                    m1, m2 = st.columns(2)
                    with m1:
                        st.metric("Deepfake Probability", f"{fake_prob:.2%}")
                    with m2:
                        st.metric("Authentic Probability", f"{real_prob:.2%}")

                except Exception as e:
                    st.error(f"GradCAM error: {e}")

with tab2:
    st.markdown("### 📄 Document Forgery GradCAM")
    st.markdown("<p style='color:#64748b;'>Upload an ID document to see WHERE the AI detected tampering</p>", unsafe_allow_html=True)

    doc_file = st.file_uploader("Upload ID Document", type=['jpg', 'jpeg', 'png'], key="gradcam_doc")

    if doc_file:
        image = Image.open(doc_file).convert("RGB")

        if st.button("🔬 Analyze Document with GradCAM", type="primary", use_container_width=True, key="btn_doc"):
            with st.spinner("Generating GradCAM visualization..."):
                try:
                    ...
                    color = '#dc2626' if is_forged else '#059669'
                    result_text = '❌ FORGERY DETECTED' if is_forged else '✅ AUTHENTIC DOCUMENT'
                    ...
                    <div style="background:#0f172a;border:2px solid {color};border-radius:12px;
                        ...
                        <p style="color:{color};font-size:22px;font-weight:bold;margin:0;">{result_text}</p>
                        <p style="color:#64748b;margin:4px 0 0 0;">
                            Forgery Probability: {forged_prob:.2%} | Authentic: {authentic_prob:.2%}
                        </p>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    with c2:
                        st.markdown("**Forgery Heatmap**")
                        st.image(heatmap, use_column_width=True)
                        st.caption("🔴 Red = Tampered region | 🔵 Blue = Authentic region")

                    with c3:
                        st.markdown("**Overlay**")
                        st.image(overlay, use_column_width=True)
                        st.caption("Highlighted suspicious regions on document")

                    st.divider()
                    st.markdown("#### 📊 Confidence Breakdown")
                    m1, m2 = st.columns(2)
                    with m1:
                        st.metric("Forgery Probability", f"{forged_prob:.2%}")
                    with m2:
                        st.metric("Authentic Probability", f"{authentic_prob:.2%}")
...
<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:8px;padding:16px;">
    ...
    see exactly WHY a document or face was flagged, not just that it was flagged.
    </p>
</div>
""", unsafe_allow_html=True)