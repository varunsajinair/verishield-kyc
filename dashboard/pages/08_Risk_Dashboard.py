...
<div style="background:linear-gradient(135deg,#0d1b2e,#1a2744);border-radius:16px;
    ...
    <h1 style="margin:0;color:white;font-size:28px;">Risk Dashboard</h1>
    <p style="color:#64748b;margin:6px 0 0 0;font-size:13px;">
        Combined risk scores from deepfake detection and face match checks
    </p>
</div>
""", unsafe_allow_html=True)
...
with m2: st.metric("Critical", len(df[df['alert_level'] == 'CRITICAL']) if not df.empty else 0)
with m3: st.metric("High Risk", len(df[df['alert_level'] == 'HIGH RISK']) if not df.empty else 0)
...
if df.empty:
    st.info("No verification data yet — showing simulated data for demo.")
    np.random.seed(42)
    n = 100
    df = pd.DataFrame({
...
st.markdown("### Platform Risk Score")
...
fig_gauge = go.Figure(go.Indicator(
    ...
    value=risk_pct,
    domain={'x': [0, 1], 'y': [0, 1]},
    title={'text': "Avg Risk Score", 'font': {'color': 'white', 'size': 16}},
...
if not high_risk.empty:
    ...
    st.success("No high risk verifications in current data.")
...
<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:8px;padding:16px;">
    <p style="color:#94a3b8;margin:0;font-size:13px;line-height:1.7;">
        <b style="color:white;">Thresholds:</b> Risk score above 0.6 → auto-rejected. 
        Between 0.3–0.6 → flagged for manual review. Below 0.3 → auto-approved. 
        Score is a weighted combination of deepfake probability and face match similarity.
    </p>
</div>
""", unsafe_allow_html=True)