...
<div style="background:linear-gradient(135deg,#0d1b2e,#1a2744);border-radius:16px;
    ...
    Combined risk scores across document, face and match checks — real-time threat intelligence
    </p>
</div>
""", unsafe_allow_html=True)
...
# Simulated risk data for demo if no real data
if df.empty:
    ...
    df = pd.DataFrame({
        ...
        'document_confidence': np.random.beta(2, 5, n),
        'face_confidence': np.random.beta(2, 5, n),
        'match_score': np.random.beta(5, 2, n),
        'overall_risk_score': np.random.beta(2, 5, n),
        'document_result': np.random.choice(['AUTHENTIC', 'AUTHENTIC', 'AUTHENTIC', 'SUSPICIOUS', 'FORGED'], n),
...
with col1:
    st.markdown("#### 📄 Document Risk")
    doc_risk = df['document_confidence'].mean() if 'document_confidence' in df.columns else 0.2
    fig_doc = go.Figure(go.Indicator(
        mode="gauge+number",
        value=doc_risk * 100,
        title={'text': "Document Forgery Risk", 'font': {'color': 'white', 'size': 12}},
        gauge={
            'axis': {'range': [0, 100], 'tickfont': {'color': 'white'}},
            'bar': {'color': '#dc2626' if doc_risk > 0.6 else '#f97316' if doc_risk > 0.3 else '#059669'},
            'bgcolor': '#0f172a',
            'steps': [
                {'range': [0, 30], 'color': 'rgba(5,150,105,0.2)'},
                {'range': [30, 60], 'color': 'rgba(249,115,22,0.2)'},
    ...
    fig_doc.update_layout(paper_bgcolor='#0f172a', font=dict(color='white'), height=250, margin=dict(t=40, b=0))
    ...
    for _, row in high_risk.head(10).iterrows():
        ...
        <div style="background:#0f172a;border-left:4px solid {color};
            ...
            <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;">
                ...
                <span style="color:#fbbf24;">Doc: {row.get('document_result', 'N/A')}</span>
...
<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:8px;padding:16px;">
    ...
    💡 <b style="color:#cbd5e1;">Risk Scoring:</b> VeriShield combines document forgery risk, 
    deepfake probability, and face match score into a unified risk score. 
    Scores above 0.6 trigger automatic rejection. Scores between 0.3-0.6 
    go to manual review. Below 0.3 are auto-approved — same logic used by 
