...
<div style="background:linear-gradient(135deg,#0d1b2e,#1a2744);border-radius:16px;
    ...
    <h1 style="margin:0;color:white;font-size:28px;">Compliance Report</h1>
    <p style="color:#64748b;margin:6px 0 0 0;font-size:13px;">
        Generate a PDF KYC verification report for a customer
    </p>
</div>
""", unsafe_allow_html=True)
...
with col2:
    ...
    face_result = st.selectbox("Deepfake Check Result", ["AUTHENTIC", "SUSPICIOUS", "DEEPFAKE"])
...
officer = st.text_input("Compliance Officer", value="")
...
def generate_pdf_report(data):
    ...
    pdf.set_xy(10, 10)
    pdf.cell(190, 10, 'VeriShield AI - KYC Verification Report', 0, 1, 'C')
    ...
    results_items = [
        ('Deepfake Detection', f"{data['face_result']} (Confidence: {data['face_confidence']:.0%})"),
        ...
        ('KYC Decision', data['overall_result']),
    ]
    ...
    pdf.set_font('Arial', 'B', 12)
    pdf.set_fill_color(230, 230, 230)
    pdf.cell(190, 8, 'DISCLAIMER', 0, 1, 'L', True)
    pdf.ln(2)
    pdf.set_font('Arial', '', 9)
    disclaimer = ("This report was generated by VeriShield AI using EfficientNet-B0 deepfake detection "
                  "and face matching. Results are based on automated analysis and should be reviewed "
                  "by a qualified compliance officer before any final decision is made.")
    ...
    pdf.set_xy(10, 275)
    pdf.cell(190, 6, 'VeriShield AI | EfficientNet-B0 | PostgreSQL Audit Trail', 0, 1, 'C')
    pdf.set_xy(10, 283)
...
if st.button("Generate Compliance Report", type="primary", use_container_width=True):
    ...
    icon = '✅' if overall_result == 'APPROVED' else '❌' if overall_result == 'REJECTED' else '⚠️'
