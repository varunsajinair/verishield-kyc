...
<div style="background:linear-gradient(135deg,#0d1b2e,#1a2744);border-radius:16px;
    ...
    Upload ID document and selfie — deepfake detection + face match verification
    </p>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    st.markdown("<p style='color:#64748b;'>Upload ID document and selfie — AI checks for deepfake + face match</p>", unsafe_allow_html=True)
    ...
    if st.button("🔍 Run Full KYC Verification", use_container_width=True, type="primary"):
        if not doc_file or not selfie_file:
            ...
            with st.spinner("Running AI verification — deepfake detection + face match..."):
                try:
                    doc_file.seek(0)
                    selfie_file.seek(0)
                    files = {
...
with tab2:
    st.markdown("### 🧬 Deepfake Face Detection")
...
<div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:8px;padding:16px;">
    ...
    💡 <b style="color:#cbd5e1;">How it works:</b> Deepfake check uses EfficientNet-B0 trained on 140K 
    real/fake faces (99.28% accuracy). Face match uses histogram correlation to compare 
    ID photo vs selfie. All results logged to PostgreSQL for compliance audit trail.
    </p>
</div>
""", unsafe_allow_html=True)